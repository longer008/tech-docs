...
- **CSS Anchor Positioning**：元素定位相对于锚点元素，而非视口
- **`@starting-style`**：进入动画（从无到有的过渡）

### 面试新增考点

Q: **容器查询和媒体查询的区别？**

A: 媒体查询基于视口宽度（全局响应），容器查询基于父容器宽度（组件级响应）。组件在不同布局位置（侧栏/主区域）可有不同样式，不再依赖页面级断点。这是组件化开发的重大进步。
