...
### TypeScript 6.0（2026-04）

TypeScript 6.0 是桥接版本，为 TS 7.0 Go 重写做准备：

- 性能提升：通过内部优化减少编译时间
- `--moduleResolution bundler` 成为推荐默认值
- Node.js ESM 支持改进

**面试考点**：

Q: **TypeScript 6.0 为什么被称为"桥接版本"？**

A: TS 6.0 的核心目标是平滑过渡到 TS 7.0（使用 Go 语言重写编译器）。它保持 API 兼容性，同时内部进行优化。TS 7.0 预计将显著提升编译速度（Go 原生性能 vs Node.js），6.0 确保升级路径顺畅。

Q: **TypeScript Go 重写有什么好处？**

A: 核心编译器从 TypeScript/Node.js 重写为 Go：
- 编译速度预计提升 5-10 倍
- 内存占用显著降低
- 语言服务响应更快
- 保持与现有代码完全兼容
