...
> Vue 3.6 目前处于 beta 阶段，核心变化是响应式系统基于 **alien-signals** 重构。

### 1. alien-signals 响应式引擎

Vue 3.6 将响应式系统的底层实现替换为 **alien-signals**（由 Vue 核心团队成员 johnsoncodehk 开发），带来显著的性能和内存改善：

- **依赖追踪算法重构**：采用基于版本号（version counter）的增量追踪，避免每次更新都重建依赖图
- **内存占用降低**：减少中间对象创建，依赖关系用更紧凑的数据结构存储
- **更快的批量更新**：改进调度器，减少不必要的重复计算
- **代码量更小**：响应式核心实现代码从数千行压缩到数百行

```typescript
// alien-signals 的核心思路：版本号追踪
// 每个响应式值有一个版本号，依赖收集时记录版本
// 触发更新时只有版本号变化的依赖才重新执行
// 相比 Vue 3.5 的 WeakMap 依赖图，内存和速度都有改善
```
