...
| **Tauri** | 轻量级 Rust 桌面应用框架 | [官方文档](https://tauri.app/) |
| **跨平台最佳实践** | 桌面应用开发经验总结 | 本文档 |
...
Tauri 是一个使用 Rust 构建后端、Web 技术构建前端的轻量级桌面应用框架。

**核心特点**：
- 包体积小（约 2-5MB vs Electron 100MB+）
- 内存占用低（Rust 后端高效）
- 安全性强（Rust 内存安全、细粒度权限控制）
...
**知名应用**：Clash Verge、Warp Terminal、Zed Editor
...
- **Tauri 2.0**：已支持移动端（iOS/Android），成为真正的跨平台方案
- **Electron 33+**：继续优化性能，但包体积和内存问题仍是主要劣势
- **Zed Editor**：使用 Rust + Tauri 技术栈的编辑器，证明 Tauri 在大型应用中的可行性

### 面试新增考点

Q: **Tauri 和 Electron 怎么选？**

A: Electron 生态成熟、插件丰富、学习成本低（纯 Web 技术栈），适合快速开发内部工具。Tauri 包体积小（~3MB vs ~150MB）、内存低、安全性高（Rust 后端），适合面向用户的商业产品。核心取舍：开发便利性 vs 产品性能。
